...
void						Ia_player::play(Grid &grid, unsigned int &x, unsigned int &y)
{
	...
	for (y_ia = 0; y_ia < SIZE_GRID; y_ia++)
		...
		{
			if (ia_player.m_grid.getValue(c, x_ia, y_ia) == false || (c & (RELEVANT | RELEVANT_IA)) == 0 || GET_VAL(c) != EMPTY_CELL ||
...
int							Ia_player::min(int const depth, unsigned int const y, unsigned int const x, int prev_branch, int weight_ia, int weight_player)
{
	...
	for (y_ia = 0; y_ia < SIZE_GRID; y_ia++)
		for (x_ia = 0; x_ia < SIZE_GRID; x_ia++)
		{
			if (this->m_grid.getValue(c, x_ia, y_ia) == false || (c & (RELEVANT | RELEVANT_IA)) == 0 || GET_VAL(c) != EMPTY_CELL ||
